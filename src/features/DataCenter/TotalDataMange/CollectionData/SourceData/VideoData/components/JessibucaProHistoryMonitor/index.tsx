...
const JessibucaProHistoryMonitor: React.FC<Props> = ({ id, code, placeId, timeRange }) => {
  console.debug('JessibucaProHistoryMonitor')
  ...
  useEffect(() => {
    ...
    async function main() {
      ...
      const _videoList = await getAllCamera({ placeId, text: search, deviceSource: 0 }, signal)
      setAllVideoList(_videoList.filter((item: any) => item.deviceSource === 0))
    }
    main()
    return () => {
      signal && signal.abort()
...
export default JessibucaProHistoryMonitor