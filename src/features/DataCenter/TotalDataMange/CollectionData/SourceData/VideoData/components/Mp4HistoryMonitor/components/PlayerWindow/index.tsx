...
import Mp4MultiSegment, { Mp4MultiSegmentRefProps } from "component/Mp4MultiSegment";
import dayjs from "dayjs";
import { YMDHms } from "helper";
import { PlaySpeedDict } from "helper/dictionary";
import Mp4PlayerSegments from "helper/player/Mp4PlayerSegments";
...
const PlayerWindow: React.FC<Props> = ({ className, device, timeRange, onClick, onLock, onUnLock }) => {
  ...
  const multiSegmentRef = useRef<Mp4MultiSegmentRefProps>(null)
  ...
  const [player, setPlayer] = useState<Mp4PlayerSegments>()
  ...
  useEffect(() => {
    let ctr: AbortController
    let playerSegments: Mp4PlayerSegments
    ...
    const onPlay = () => {
      setTimeout(() => setIsPlay(true), 2000)
    }
    ...
    if (videoRef.current && device && palyerSegmentList.length) {
      playerSegments = new Mp4PlayerSegments(videoRef.current, {
  ...
  }, [device, palyerSegmentList, videoSegmentChannel])
  ...
  return (
    ...
    >
      <section className={styles.videoContent}>
        ...
        <video className={styles.video} ref={videoRef} style={videoStyle} muted />
      </section>
      {videoAlt &&
