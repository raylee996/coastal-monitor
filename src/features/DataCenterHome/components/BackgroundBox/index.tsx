...
import mapSrc from 'images/dataCenter/data_home_map1.png'
import nightSrc from 'images/dataCenter/data_home_map.jpg'
import markerIcon from "images/mapIcon/markerIcon.png";
import { getSpeedLength, mapMarkerA, mapMarkerB } from "helper";
import { getAisColor, getMapFusionTargetIcon, getMapRadarTargetIcon, markerAIcon, markerBIcon } from "helper/mapIcon";
...
const BackgroundBox: React.FC<Props> = ({ isWhite }) => {
  console.debug('BackgroundBox')
  ...
  useEffect(() => {
    let Imglayer: any
    let topRightMarker: any
    let bottomLeftMarker: any
    const topLeftPoint = L.latLng(22.562199699881887, 113.71340199424486),
      topRightPoint = L.latLng(22.562199699881887, 114.1196075053694),
      bottomLeftPoint = L.latLng(22.35062558684138, 113.71340199424486),
      bottomRightPoint = L.latLng(22.35062558684138, 114.1196075053694);

    const imageOverlaySrc = isWhite ? mapSrc : nightSrc

    if (map2d) {
      // 创建可以拖拽的UI设计图。
      const imgEle = document.createElement('img')
      imgEle.src = markerIcon
      // imgEle.width = 25
      // imgEle.height = 41
      imgEle.width = 0
      imgEle.height = 0

      // const icon = L.divIcon({ html: imgEle, iconSize: [25, 41] })
      const icon = L.divIcon({ html: imgEle, iconSize: [0, 0] })

      topRightMarker = L.marker(topRightPoint, { draggable: true, icon }).addTo(map2d.map);

      bottomLeftMarker = L.marker(bottomLeftPoint, { draggable: true, icon }).addTo(map2d.map);

      Imglayer = L.imageOverlay.rotated(imageOverlaySrc, topRightMarker, bottomLeftMarker, {
        opacity: 1,
        interactive: true,
  ...
  useEffect(() => {
    ...
    if (map2d) {
      ...
      aisLayerGroup.forEach(item => {
        ...
        if (item.extraData.riskLevel !== 4) {

          let color = '#25F076'
          let borderColor = '#404040'
          // riskLevel 1高风险 2中风险 3低风险
          switch (item.extraData.riskLevel) {
            case 1:
              color = '#ff2f25'
              break;
            case 2:
