...
//伴随分析
const Bansui: React.FC<Props> = ({eventId, eventType, extraParams}) => {
  ...
  return <>
    <div className={styles.bottom}>
      ...
      <div className={styles.trailDetail}>
        ...
        <ul>
          {
            trackDetail && trackDetail.map((item: any,index:number) => {
              return <li key={index}>
                <div>分析数据：{item.dataContent}</div>
                <div>伴随数据：{item.dataContent2}</div>
