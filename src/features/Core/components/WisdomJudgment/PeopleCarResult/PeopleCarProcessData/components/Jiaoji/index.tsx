...
//交集运算、并集运算，差集运算
const Jiaoji: React.FC<Props> = ({eventId, eventType, extraParams}) => {
  ...
  return <>
    <div className={styles.bottom}>
      ...
      <div className={styles.trailDetail}>
        {eventType==='14' && <p className={styles.title}>交集运算</p>}
        {eventType==='15' && <p className={styles.title}>并集运算</p>}
        {eventType==='17' && <p className={styles.title}>差集运算</p>}
        <ul>
          {/*需要单独调用接口请求数据*/}
          {
            trackDetail && trackDetail.map((item: any) => {
              return <li key={item.id}>
                <div>条件1：{item.dataContent}</div>
                <div>数据内容：{item.dataContent}</div>
                <div>碰撞点位数：{item.pointAddress}</div>
                <div>伴随次数：{item.pointCount}</div>
              </li>
            })
          }
        </ul>
